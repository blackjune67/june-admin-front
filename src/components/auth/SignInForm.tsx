...
export default function SignInForm() {
  ...
  const validate = (value: FormState): FormErrors => {
    ...
    if (!value.email.trim()) {
      nextErrors.email = t("auth.signIn.emailRequired");
    } else if (!emailRegex.test(value.email.trim())) {
      nextErrors.email = t("auth.signIn.emailInvalid");
    }

    if (!value.password.trim()) {
      nextErrors.password = t("auth.signIn.passwordRequired");
    }
  ...
  const getErrorMessage = () => {
    ...
    if (loginMutation.error instanceof ApiError) {
      switch (loginMutation.error.errorCode) {
        case "UNAUTHORIZED":
        ...
        default:
          return locale === "en"
            ? t("auth.signIn.requestError")
            : loginMutation.error.message;
      }
    }

    return t("auth.signIn.requestError");
  };
          ...
          <div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-6">
                ...
                <div>
                  <Button className="w-full" size="sm" disabled={loginMutation.isPending}>
